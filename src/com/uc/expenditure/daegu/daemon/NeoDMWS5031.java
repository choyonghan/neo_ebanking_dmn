/**
 *  주시스템명 : 대구시 통합자금관리
 *  업  무  명 : e세출 가상계좌(일상경비) 세출한도계좌의 이체결과파일 생성처리
 *  기  능  명 : e세출 가상계좌(일상경비) 세출한도계좌의 이체결과파일 생성처리
 *  클래스  ID : NeoDMWS5031
 *  변경  이력 :
 * ------------------------------------------------------------------------
 *  작성자         소속                 일자            Tag                 내용
 ...
 * 흐름 : [세출->금고은행]
 * <br />
 * 1.  [금고요청구분코드(STBX_DMND_DV_CD) / 수납요청구분코드(RCVMT_DMND_DV_CD)] 값이 "20" 인 건에 대해서 정상건 중에서
 * <br />
 * 금고은행에 보낼 세출한도계좌에 대한 이체결과파일을 생성하는 데몬이다.
 * <br />
 *
 */
public class NeoDMWS5031 implements Runnable {
    @SuppressWarnings("unchecked")
    static Logger logger = Logger.getLogger(NeoDMWS5031.class);
    ...
    public static void main(String args[]) {
        ...
        DOMConfigurator.configure(NeoDMWS5031.class.getResource("/conf/log4j.xml"));
        if( logger.isDebugEnabled()) {
        	logger.debug("===== [" + NeoDMWS5031.class.getSimpleName() + "] 시작 =====");
        }
        NeoDMWS5031 hello = new NeoDMWS5031();
        ...
        if( logger.isDebugEnabled()) {
        	logger.debug("===== [" + NeoDMWS5031.class.getSimpleName() + "] 끝 =====");
        }
    }
    ...
    public int mainLoop() {
        ...
        try {
        	...
        	List<MyMap> trgetList = session.selectList("NeoMapper5031.trgetList", param);
            ...
            fileDvcd = appRes.getString("ServiceLimAccDvCd"); // 세출한도계좌 CMS전송 파일 업무구분
        	...
        	for( MyMap trgetData : trgetList ) {
        		...
        		// 2. 금고은행 전송 파일 생성 H:모계좌 / D:한도계좌
        		// 2-1. 반납계좌검증
        		/*
        		String ecrpActno = trgetData.getString("ECRP_ACTNO"); // 가상계좌T->반납계좌번호
            	param.setString("ECRP_ACTNO", ecrpActno);

            	MyMap actnoMap = (MyMap) session.selectOne("NeoMapperCommon.selectDrwRealActno", param);
            	String aneLimAccYn = actnoMap.getString("ANE_LIM_ACC_YN"); // 세출한도계좌여부
            	*/

        		// 업무원장에 [세출한도계좌여부] 컬럼 추가함.
        		// dvc200 에서 계좌여부 판단함.

        		String ecrpActno = trgetData.getString("ECRP_ACTNO"); // 가상계좌T->반납계좌번호

        		// 입금계좌번호 변경
        		trgetData.setString("입금계좌번호", ecrpActno);

        		// 데이터맵 변경
        		dataMap = getDataMap( trgetData );

        		// 금고은행 정상건 전송을 위한 설정

        		/* 금고은행전송 정상건에 대해서 신규추가함.20221025*/
        		// 헤더
        		headerMap.setString("파일불능코드", "0000");
        		headerMap.setString("파일불능내용", "정상처리 되었습니다.");
        		// 바디
        		dataMap.setString("이체처리불능코드", "BN10501");
        		dataMap.setString("이체처리불능내용", "정상처리 되었습니다.");


        		int proc = fileWriter( trnxNo.concat("_STBX"), headerMap, dataMap, trailerMap );
        		if( proc == 0 ) {

        			trgetData.setString("STBX_TRSM_TRGT_DV_CD", "Y"); // 금고전송대상구분코드(Y:전송대상)

            		// 이력등록
            		session.update("NeoMapper5031.insert2547h", trgetData);

            		// 금고전송대상구분코드 수정
            		session.update("NeoMapper5031.update2547", trgetData);
    ...
    private MyMap getHeaderMap( MyMap trgetData ) throws Exception {
        ...
        headerMap.setString("파일업무구분", fileDvcd);
        ...
        // 헤더 공란에 신규추가함. 20221107
        //RTN_DND_DEPT_CD - 반납요구부서코드
        headerMap.setString("공란", trgetData.getString("RTN_DND_DEPT_CD"));
    ...
    private MyMap getDataMap( MyMap trgetData ) throws Exception {
        ...
        dataMap.setString("파일업무구분", fileDvcd);
...
//		DataForm.setString("이체처리불능코드", "BE01513");	// 오류샘플
    ...
    private MyMap getTrailer( MyMap trgetData ) throws Exception {
        ...
        TrailerMap.setString("파일업무구분", fileDvcd);
        ...
        try {
            ...
            if (sqNo == 1) {
                ...
                if( "EA".equalsIgnoreCase( dvcd)) {
                    ...
                    mapForm.setLong("FILESQNO", 800001);//5030과 다름
                }else {

                    // VR
                    sqNo = 900001;
                    mapForm.setLong("FILESQNO", 900001);
                }
